...
def gpu_image_processing(image):
    ...
    end_time = time.time()
    execution_time = end_time - start_time
    return gpu_gray_image, execution_time
...
def ExecuteCpuWithResult():
    _ , cpu_execution_time = cpu_image_processing(image)
    return cpu_execution_time

def ExecuteGpuWithResult():
    _ , gpu_execution_time = gpu_image_processing(image)
